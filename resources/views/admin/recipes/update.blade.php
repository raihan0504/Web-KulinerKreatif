            ...
            <section class="mb-8">
                ...
                <form method="POST" action="{{ route('recipes.update', $recipe->id) }}" enctype="multipart/form-data">
                    ...
                    @method('PUT')
                     ...
                     @if ($recipe->image)
                     <div class="mb-4">
                         <img src="{{ asset('storage/' . $recipe->image) }}" alt="{{ $recipe->title }}" class="w-48 h-32">
                     </div>
                 @endif

                    <div class="mb-4">
                        <label for="title" class="block text-sm font-medium text-gray-300">Recipe Name</label>
                        <input type="text" name="title" id="title" value="{{ $recipe->title }}" class="mt-1 block w-full bg-gray-700 text-white border-gray-600 rounded-md shadow-sm">
                    </div>
                    <div class="mb-4">
                        <label for="description" class="block text-sm font-medium text-gray-300">Description</label>
                        <textarea name="description" id="description" class="mt-1 block w-full bg-gray-700 text-white border-gray-600 rounded-md shadow-sm">{{ $recipe->description }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label for="ingredients" class="block text-sm font-medium text-gray-300">Ingredients</label>
                        <textarea name="ingredients" id="ingredients" class="mt-1 block w-full bg-gray-700 text-white border-gray-600 rounded-md shadow-sm">{{ $recipe->ingredients }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label for="instructions" class="block text-sm font-medium text-gray-300">Instructions</label>
                        <textarea name="instructions" id="instructions" class="mt-1 block w-full bg-gray-700 text-white border-gray-600 rounded-md shadow-sm">{{ $recipe->instructions }}</textarea>
                    </div>
